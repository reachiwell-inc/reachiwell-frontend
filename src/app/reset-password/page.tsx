...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    // Get reset code from URL params or localStorage
    const codeFromUrl = searchParams.get("code");
    const codeFromStorage = localStorage.getItem("passwordResetCode");

    if (codeFromUrl) {
      setResetCode(codeFromUrl);
      localStorage.setItem("passwordResetCode", codeFromUrl);
    } else if (codeFromStorage) {
      setResetCode(codeFromStorage);
    } else {
      // No reset code found, redirect to forgot password page
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        setSuccess(true);
        // Clear reset code from localStorage
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="relative">
                  <input
                    ...
                    required
                    className={`w-full px-6 py-4 pr-14 rounded-[30px] border text-[#0B2220] text-base font-normal focus:outline-none focus:ring-2 focus:ring-[#E87954]/20 placeholder:text-[#9CA3AF] ${error ? "border-red-500 focus:border-red-500" : "border-[#C2D6D4] focus:border-[#E87954]"
                      }`}
                  />
                  <button
                    type="button"
...
export default function ResetPasswordPage() {
  return (
    <Suspense fallback={
      <div className="flex flex-col w-full min-h-screen bg-[#F3FAF9] items-center justify-center">
